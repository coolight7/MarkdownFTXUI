...
std::string collect_text(ASTNode const& root) {
    ...
    while (!stack.empty()) {
        ...
        if (n->type == NodeType::SoftBreak) {
            result += ' ';
        }
        if (n->type == NodeType::HardBreak) {
            result += '\n';
        }
...
ftxui::Decorator link_style(bool is_focused, ftxui::Decorator base, Theme const& theme) {
    if (is_focused) {
        return base | ftxui::underlined | ftxui::inverted;
    }
...
ftxui::Elements build_children(
    ...
    for (auto const& child : node.children) {
        result.push_back(build_node(child, depth, qd, mqd, links, focused_link, theme));
    }
    return result;
}

// Collect inline children into a single hbox (for paragraphs, etc.)
ftxui::Element build_inline_container(
    ASTNode const& node,
    int            depth,
    int            qd,
    int            mqd,
    ...
    for (auto const& child : node.children) {
        parts.push_back(build_node(child, depth, qd, mqd, links, focused_link, theme));
    }
    if (parts.empty()) {
        return ftxui::text("");
    }
...
void collect_inline_words(
    ...
    if (depth > kMaxDepth) {
        ...
        if (!text.empty()) {
            words.push_back(ftxui::text(text) | style);
        }
        return;
    }
    for (auto const& child : node.children) {
        switch (child.type) {
            ...
            case NodeType::CodeInline:
                words.push_back(
                    ftxui::text(normalize_emoji_width(child.text)) | theme.code_inline | style
                );
                break;
            default:
                words.push_back(
                    build_node(child, depth, qd, mqd, links, focused_link, theme) | style
                );
                break;
        }
    }
...
ftxui::Element build_wrapping_container(
    ASTNode const& node,
    int            depth,
    int            qd,
    int            mqd,
...
ftxui::Element build_list_item(
    ...
    for (auto const& child : node.children) {
        if (first_para && (child.type == NodeType::Paragraph || child.type == NodeType::Text)) {
            // First paragraph: render with wrapping, bullet/number prefix
            auto content
                = build_wrapping_container(child, depth, qd, mqd, links, focused_link, theme);
            ...
            rows.push_back(build_node(child, depth, qd, mqd, links, focused_link, theme));
        }
    }
    if (rows.empty()) {
...
ftxui::Element build_document(
    ...
    auto children = build_children(node, depth, qd, mqd, links, focused_link, theme);
    if (children.empty()) {
        return ftxui::text("");
    }
...
ftxui::Element build_heading(
    ...
    auto content = build_wrapping_container(node, depth, qd, mqd, links, focused_link, theme);
    if (node.level == 1) {
        return content | theme.heading1;
    }
    if (node.level == 2) {
        return content | theme.heading2;
    }
    return content | theme.heading3;
}

ftxui::Element build_link(
    ASTNode const& node,
    int            depth,
    int            qd,
    int            mqd,
    ...
    bool is_focused = is_next_link_focused(links, focused_link);
    auto el         = build_inline_container(node, depth, qd, mqd, links, focused_link, theme)
...
ftxui::Element build_bullet_list(
    ASTNode const& node,
    int            depth,
    int            qd,
    int            mqd,
    ...
    for (auto const& child : node.children) {
        items.push_back(
            build_list_item(child, depth + 1, qd, mqd, "\u2022 ", links, focused_link, theme)
        );
    }
    return ftxui::vbox(std::move(items));
}

ftxui::Element build_ordered_list(
    ASTNode const& node,
    int            depth,
    int            qd,
    int            mqd,
    ...
    for (auto const& child : node.children) {
        items.push_back(build_list_item(
            child,
            depth + 1,
            qd,
            mqd,
...
ftxui::Element build_blockquote(
    ASTNode const& node,
    int            depth,
    int            qd,
    int            mqd,
    ...
    auto content
        = ftxui::vbox(build_children(node, depth, qd + 1, mqd, links, focused_link, theme));
...
ftxui::Element build_code_block(ASTNode const& node, Theme const& theme) {
    ...
    // 无边框块样式: 每行用 filler 横向铺满容器宽度, 使 theme.code_block
    // 的背景色 (block color) 能覆盖整个代码块区域。
    ...
    while (start <= code.size()) {
        ...
        if (end == std::string_view::npos) {
            lines.push_back(ftxui::hbox({
                ftxui::text(std::string(code.substr(start))),
                ftxui::filler(),
            }));
            break;
        }
    ...
    if (lines.empty()) {
        lines.push_back(ftxui::hbox({ftxui::text("")}));
    }
    auto content = ftxui::vbox({
                   ...
                   | theme.code_block;
    if (!node.info.empty()) {
        // 语言名显示在代码块顶部, 同样无边框, 与代码区共享背景色
        return ftxui::vbox({
            ftxui::hbox({
                ftxui::text(" " + node.info + " ") | ftxui::dim,
                ftxui::filler(),
            }) | theme.code_block,
            content,
        });
...
ftxui::Element build_image(
    ...
    auto alt = build_inline_container(node, depth, qd, mqd, links, focused_link, theme);
    return ftxui::hbox({
...
ftxui::Element build_table(
    ...
    for (auto const& row_node : node.children) {
        if (row_node.type != NodeType::TableRow) {
            continue;
        }
    ...
    auto border_line =
        [&](std::string_view left, std::string_view mid, std::string_view right, std::string_view h
        ) {
            std::string s(left);
    ...
    auto render_row = [&](std::vector<ftxui::Element> const& row, bool is_header) {
        ...
        for (size_t c = 0; c < col_widths.size(); ++c) {
            ftxui::Element content = (c < row.size()) ? row[c] : ftxui::text("");
            char           align   = (c < node.alignments.size()) ? node.alignments[c] : 'l';
            ftxui::Element aligned;
            switch (align) {
                case 'c':
                    aligned = ftxui::hbox({ftxui::filler(), std::move(content), ftxui::filler()});
                    break;
                case 'r':
            ...
            if (is_header) {
                aligned = aligned | theme.table_header;
            }
            parts.push_back(ftxui::text(" "));
            parts.push_back(
                std::move(aligned) | ftxui::size(ftxui::WIDTH, ftxui::EQUAL, col_widths[c])
            );
            parts.push_back(ftxui::text(" "));
            parts.push_back(ftxui::text("\u2502") | theme.table_border);
        }
...
ftxui::Element build_node(
    ...
    switch (node.type) {
        case NodeType::Document:
            return build_document(node, depth, qd, mqd, links, focused_link, theme);
        case NodeType::Heading:
            return build_heading(node, depth, qd, mqd, links, focused_link, theme);
        case NodeType::Paragraph:
            return build_wrapping_container(node, depth, qd, mqd, links, focused_link, theme);
        case NodeType::Strong:
            return build_inline_container(node, depth, qd, mqd, links, focused_link, theme)
                   | ftxui::bold;
        case NodeType::Emphasis:
            return build_inline_container(node, depth, qd, mqd, links, focused_link, theme)
                   | ftxui::italic;
        ...
        case NodeType::BulletList:
            return build_bullet_list(node, depth, qd, mqd, links, focused_link, theme);
        case NodeType::OrderedList:
            return build_ordered_list(node, depth, qd, mqd, links, focused_link, theme);
        case NodeType::ListItem:
            return build_list_item(node, depth, qd, mqd, "\u2022 ", links, focused_link, theme);
        case NodeType::BlockQuote:
            return build_blockquote(node, depth, qd, mqd, links, focused_link, theme);
        case NodeType::CodeInline:
