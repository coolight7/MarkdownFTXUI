...
std::string truncateToWidth(std::string_view s, int maxWidth) {
    ...
    if (utf8_display_width(s) <= maxWidth) {
        return std::string(s);
    }
    if (maxWidth == 1) {
        return "\xE2\x80\xA6"; // "…"
    }
    auto        bytePos = visual_col_to_byte(s, maxWidth - 1);
...
std::string padToWidth(std::string_view s, int width) {
    std::string out(s);
    int         cur = utf8_display_width(out);
    while (cur < width) {
        out += ' ';
        ++cur;
    }
...
BoxGeom buildBox(const MermaidStateNode& node, int maxInterior) {
    ...
    for (const auto& l : labelLines) {
        interior = std::max(interior, utf8_display_width(l));
    }
    ...
    for (int L = 0; L <= layout.maxLayer; ++L) {
        if (L < layout.maxLayer) {
            for (const auto& e : bands[L]) {
                bandLabelW[L] = std::max(bandLabelW[L], utf8_display_width(e.label));
            }
        }
    }
...
std::function<Color(std::string_view)> diagramNodeColor(Theme const& theme) {
    return [&theme](std::string_view id) -> Color {
        if (id.ends_with("_in_progress")) {
            return theme.diagram_running;
        }
        if (id.ends_with("_completed")) {
            return theme.diagram_done;
        }
        if (id.ends_with("_failed")) {
            return theme.diagram_failed;
        }
        if (id.ends_with("_pending")) {
            return theme.diagram_pending;
        }
        ...
        for (int L = 0; L <= layout.maxLayer; ++L) {
            ...
            if (L < layout.maxLayer) {
                if (bands[L].empty()) {
                    ...
                    int lbl = 0;
                    for (const auto& e : bands[L]) {
                        lbl = std::max(lbl, utf8_display_width(e.label));
                    }
                    lrWidth += 3 + lbl;
                }
            }
